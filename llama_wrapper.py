...
class LlamaWrapper:
    ...
    def _run_llama(self, full_prompt: str, image_path: str | None = None, n_predict: int | None = None, seed: int | None = None, keep_model_loaded: bool = False, temperature: float = 0.7) -> str:
        if keep_model_loaded:
            try:
                ...
                err = str(e)
                if ('Connection refused' not in err) and ('Errno 111' not in err) and ('URLError' not in err):
                    print(f"[Qwen3-VL] Warning: llama-server call failed, falling back to CLI: {e}")
                else:
                    print("[Qwen3-VL] llama-server not reachable, falling back to CLI")
    ...
    def enhance_prompt(self, text: str, preset_prompt: str = "🖼️ Detailed Analysis", use_preset_tokens: bool = True, seed: int | None = None, keep_model_loaded: bool = False, temperature: float = 0.7) -> str:
        ...
        return self._run_llama(full_prompt, n_predict=n_predict, seed=seed, keep_model_loaded=keep_model_loaded, temperature=temperature)

    def describe_image(self, image_path: str, preset_prompt: str = "🖼️ Detailed Analysis", use_preset_tokens: bool = True, seed: int | None = None) -> str:
    ...
    def multimodal_enhance(self, text: str, image_path: str, preset_prompt: str = "🖼️ Detailed Analysis", use_preset_tokens: bool = True, seed: int | None = None) -> str:
